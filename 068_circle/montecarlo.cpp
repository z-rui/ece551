...
double montecarloArea(const Circle *c1, const Circle *c2)
{
	const int MONTECARLOTRIALS = 10000000;
	struct myCircle {
		struct {
			double x, y;
		} c;
	...
	myCircle mc1 = *(const myCircle *)(c1);
	myCircle mc2 = *(const myCircle *)(c2);
	double l, r, t, b;
	l = std::min(mc1.c.x - mc1.r, mc2.c.x - mc2.r);
	r = std::max(mc1.c.x + mc1.r, mc2.c.x + mc2.r);
	t = std::max(mc1.c.y + mc1.r, mc2.c.y + mc2.r);
	b = std::min(mc1.c.y - mc1.r, mc2.c.y - mc2.r);
	...
	for (int i = 0; i < MONTECARLOTRIALS; i++) {
		double x = distx(g);
		double y = disty(g);
		double d1 = hypot(x - mc1.c.x, y - mc1.c.y);
		double d2 = hypot(x - mc2.c.x, y - mc2.c.y);
		if (d1 <= mc1.r && d2 <= mc2.r)
			++successtrials;
	}
	return (double) successtrials / MONTECARLOTRIALS * (r-l) * (t-b);
}
...
void doIntersect (char * line, Circle ** circles, int ncircles) {
  ...
  double b = montecarloArea(circles[c1], circles[c2]);
  printf("monte carlo rel. error: %g\n", (a == 0.0) ? (b == 0.0) ? 0.0 : copysign(HUGE_VAL, b-a) : (b-a) / a);
}
